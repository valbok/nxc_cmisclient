<?php
/**
 * Created on: <08-June-2009 11:00:00 ar>
 *
...
class nxcCMISOEAjaxContent
{
    ...
    */
    public static function getInstance()
    {
        ...
        {
            self::$instance = new nxcCMISOEAjaxContent();
        }
    ...
    */
    public static function simplify( $obj, $params = array() )
    {
        ...
        elseif ( !( $obj instanceof nxcCMISObjectHandler ) or !$obj->hasObject() )
        {
            return array();
        }

        $ini = eZINI::instance( 'site.ini' );
        $params = array_merge( array( 'dataMap' => array(), // collection of identifiers you want to load, load all with array('all')
        ...
        $ret['name']             = $cmisObject->getTitle();
        $ret['uri']              = $cmisObject->getSelfUri();
        $ret['owner_id']         = $cmisObject->getAuthor();
        $ret['class_name']       = $cmisObject->getDocType();
        ...
        if ( $params['fetchPath'] )
        {
            foreach ( $obj->getBreadCrumbs( '' ) as $n )
            {
        ...
        {
            $ret['children_count'] = $ret['is_container'] ? count( $obj->getChildren() ) : 0;
        }
        else
        {
